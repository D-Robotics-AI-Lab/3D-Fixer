...
from threeDFixer import models, datasets, trainers
from threeDFixer.utils.dist_utils import setup_dist
...
def main(local_rank, cfg):
    ...
    for name, model in cfg.models.items():
        if not model.args.get('frozen', False):
            model_dict[name] = getattr(models, model.name)(**model.args).cuda()
        else:
            model.args.pop('frozen')
            frozen_model = getattr(models, model.name)(**model.args) 
            frozen_model = frozen_model.eval()
            frozen_model_dict[name] = frozen_model.cuda()
    ...
    # Build trainer
    trainer = getattr(trainers, cfg.trainer.name)(model_dict, dataset, **cfg.trainer.args, output_dir=cfg.output_dir, load_dir=cfg.load_dir, step=cfg.load_ckpt, frozen_models=frozen_model_dict)
