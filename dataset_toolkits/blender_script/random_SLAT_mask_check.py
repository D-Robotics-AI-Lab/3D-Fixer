...
def init_render(engine='CYCLES', resolution=512, geo_mode=False):
    ...
    bpy.context.scene.cycles.samples = 8 if not geo_mode else 1
...
def init_nodes(save_depth=False, save_normal=False, save_albedo=False, save_mist=False):
    ...
    if save_mist:
        ...
        mist_file_output.format.file_format = 'OPEN_EXR'
...
def normalize_scene(angles, vertices_trans, vertices_scale) -> Tuple[float, Vector]:
    ...
    org_rot_mode = scene.rotation_mode
    scene.rotation_mode = 'XYZ'
    scene.rotation_euler = Euler(angles)
    scene.rotation_mode = org_rot_mode
    bpy.context.view_layer.update()
    # 
    scene.matrix_world.translation -= Vector(vertices_trans)
    bpy.context.view_layer.update()
    scene.scale = scene.scale / vertices_scale
    bpy.context.view_layer.update()
    # 
    # bpy.ops.object.select_all(action="DESELECT")
    # return scale, offset
...
def main(arg):
    ...
    transforms = json.loads(arg.transforms)
    # normalize scene
    normalize_scene(transforms['angles'],
                    transforms['vertices_trans'],
                    transforms['vertices_scale'])
    ...
    for i, view in enumerate(views):
        ...
        bpy.context.scene.render.filepath = os.path.join(arg.output_folder, view['file_path'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--transforms', type=str, help='JSON string of transformation. Contains a list of {yaw, pitch, radius, fov} object.')
