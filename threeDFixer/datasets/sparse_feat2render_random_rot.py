...
class SparseFeat2RenderRandRot(StandardDatasetBase):
    ...
    def __init__(
        ...
        rot_model: str = 'dinov2_vitl14_reg_slat_enc_swin8_B_64l8_fp16',
        resolution: int = 64,
    ...
    def filter_metadata(self, metadata):
        stats = {}
        metadata = metadata[metadata[f'feature_{self.model}']==True]
    ...
    def _get_image(self, root, instance):
        ...
        if np.random.rand() < self.perturb_ratio:
            ...
            if len(trans_list) > 0:
                ...
                feats_path = os.path.join(root, 'latents_rot', self.rot_model, f'{instance}', f'{rotated_slat_name}.npz')
                feats = np.load(feats_path, allow_pickle=True)
                if 'patchtokens' not in feats:
                    rotated_slat_name = None
                    metadata = None
                    image_path_root = os.path.join(root, 'renders', instance)
    ...
    def _get_feat(self, root, instance, rotated_slat_name):
        DATA_RESOLUTION = 64
        if rotated_slat_name is None:
            ...
            feats_path = os.path.join(root, 'latents_rot', self.rot_model, f'{instance}', f'{rotated_slat_name}.npz')
            feats = np.load(feats_path, allow_pickle=True)
            coords = torch.tensor(feats['coords']).int()
            feats = torch.tensor(feats['patchtokens']).float()
    ...
    def get_instance(self, root, instance):
        ...
        feat = self._get_feat(root, instance, rotated_slat_name)
        return {
            **image,
            **feat,
        }
