...
import torch
import numpy as np
from ....modules.sparse.basic import SparseTensor, sparse_unbind, sparse_cat
    ...
    def get_cond(self, cond, neg_cond=None, **kwargs):
        ...
        if self.p_uncond > 0:
            ...
            def select(cond, neg_cond, mask):
                ...
                elif isinstance(cond, SparseTensor):
                    unbind_cond = sparse_unbind(cond, dim=0)
                    unbind_neg_cond = sparse_unbind(neg_cond, dim=0)
                    return sparse_cat([nc if m else c for c, nc, m in zip(unbind_cond, unbind_neg_cond, mask)])
                else:
                    raise ValueError(f"Unsupported type of cond: {type(cond)}")
            ...
            if not isinstance(cond, dict):
                ...
                for k in cond.keys():
                    cond[k] = select(cond[k], neg_cond[k], mask)
