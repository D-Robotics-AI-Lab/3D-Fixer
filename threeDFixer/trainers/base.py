...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.models = models
        self.frozen_models = kwargs.get('frozen_models', {})
    ...
    def prepare_dataloader(self, **kwargs):
        ...
        self.dataloader = DataLoader(
            ...
            num_workers = int(os.environ.get("DEBUG_NUM_WORKERS", 16)),
            pin_memory=False,
            drop_last=True,
            persistent_workers=int(os.environ.get("DEBUG_NUM_WORKERS", 16)) > 0,
    ...
    def decode_sample(self, sample):
        ...
        Convert a sample to an image.
        """
        if hasattr(self.dataset, 'decode_sample'):
            return self.dataset.decode_sample(sample)
        else:
            return sample

    @torch.no_grad()
    def snapshot_dataset(self, num_samples=int(os.environ.get("DEBUG_SNAP_DATASET", 100))):
        """
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=int(os.environ.get("DEBUG_SNAP", 64)), batch_size=1, verbose=False):
        """
        ...
        for key in list(samples.keys()):
            if samples[key]['type'] == 'sample':
                ## save ply
                if not isinstance(samples[key]['value'], sp.SparseTensor):
                    ss = self.decode_sample(samples[key]['value'])
                    samples[key+"_vertices"] = {'value': ss, 'type': 'vertices'}
        ...
        if self.is_master:
            ...
            vertices_dirs = os.path.join(self.output_dir, 'samples', suffix, "vertices")
            for key in samples.keys():
                ...
                elif samples[key]['type'] in ['vertices', 'points']:
                    os.makedirs(vertices_dirs, exist_ok=True)
                    if samples[key]['type'] == 'vertices':
                        for i in range(num_samples):
                            instance_vertices_dirs = os.path.join(vertices_dirs, f'{i:03d}')
                            os.makedirs(instance_vertices_dirs, exist_ok=True)
                            coords = torch.nonzero(samples[key]['value'][i, 0] > 0, as_tuple=False)
                            resolution = samples[key]['value'].shape[-1]
                            position = coords.float() / resolution - 0.5
                            utils3d.io.write_ply(os.path.join(instance_vertices_dirs, key.replace('_vertices', '') + '.ply'), position.detach().cpu().numpy())
                    elif samples[key]['type'] == 'points':
                        for i in range(num_samples):
                            instance_vertices_dirs = os.path.join(vertices_dirs, f'{i:03d}')
                            os.makedirs(instance_vertices_dirs, exist_ok=True)
                            position = samples[key]['value'][i] / 4.4007194
                            position = torch.nan_to_num(position)
                            # position = torch.clamp(position, min=-0.5, max=0.5)
                            utils3d.io.write_ply(os.path.join(instance_vertices_dirs, key.replace('_points', '') + '.ply'), position.detach().cpu().numpy())
